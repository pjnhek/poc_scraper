...
def test_all_records_have_split_field() -> None:
    ...
    assert all(ex.split in ("train", "holdout") for ex in examples)
...
def test_all_records_have_five_axis_expected_values() -> None:
    ...
    for ex in examples:
        ...
        assert 1.0 <= ex.expected_specificity <= 5.0
...
def test_no_eval_failed_true_except_sentinel_records() -> None:
    ...
    assert all("judge-failed" in ex.coverage_cells for ex in failed)
